...
export function isTrustedDiscordUrl(value: string): boolean { try { const url = new URL(value); return url.protocol === "https:" && TRUSTED_HOSTS.has(url.hostname.toLowerCase()); } catch { return false; } }
export function isTrustedIpcSender(value: string): boolean { return isTrustedDiscordUrl(value) || value.startsWith("file://"); }
