...
import { CommandLine, isSafeMode } from "./cli";
...
function initMenuBar(win: BrowserWindow) {
    ...
    const subMenu = [
        ...
        !isSafeMode && {
            ...
            toolTip: "PulseCord will automatically restart after this operation"
        },
        isSafeMode && {
...
function buildBrowserWindowOptions(): BrowserWindowConstructorOptions {
    ...
    if (staticTitle || isSafeMode) {
        options.title = isSafeMode ? "PulseCord — Safe Mode" : "PulseCord";
    }
...
export async function createWindows() {
    ...
    if (!isSafeMode) {
        ...
        console.warn("PulseCord is running in safe mode. Plugins, themes, and renderer patches are disabled.");
    }
    ...
    AppEvents.on("appLoaded", () => {
        ...
        setTimeout(() => {
            State.store.recovery = { consecutiveRendererCrashes: 0 };
        }, 60_000).unref();
    });
    ...
    mainWin.webContents.on("render-process-gone", (_event, details) => {
        console.error("Discord renderer stopped:", details);
        if (isQuitting || details.reason === "clean-exit" || recoveringFromCrash) return;

        const now = Date.now();
        const previous = State.store.recovery;
        const isRecent = previous?.lastRendererCrashAt && now - previous.lastRendererCrashAt < 120_000;
        const consecutiveRendererCrashes = isRecent ? (previous?.consecutiveRendererCrashes ?? 0) + 1 : 1;
        ...
        if (consecutiveRendererCrashes < 2 || isSafeMode) {
            mainWin.reload();
            return;
        }
        ...
        dialog
            .showMessageBox({
                type: "warning",
                title: "PulseCord recovery",
                message: "PulseCord detected repeated renderer crashes.",
                detail: "The app will restart in Safe Mode with plugins and themes disabled so you can recover your account session and settings."
            })
            .finally(() => {
                const args = process.argv
                    .slice(1)
                    .filter(arg => arg !== "--safe-mode")
                    .concat("--safe-mode");
                app.relaunch({ args });
                app.exit(0);
            });
