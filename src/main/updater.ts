...
// Preview builds are intentionally local-only. Public updates stay disabled until the
// project has signed release artifacts and an explicit distribution policy.
const updatesEnabled = app.isPackaged && process.env.PULSECORD_ENABLE_UPDATER === "1";
const isOutdated = updatesEnabled
    ? autoUpdater
          .checkForUpdates()
