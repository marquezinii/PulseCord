...
const options = {
    ...
    "safe-mode": {
        default: false,
        type: "boolean",
        description: "Start without plugins, themes, or PulseCord renderer patches"
    },
    "windows-spoof": {
        default: false,
        type: "boolean",
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run PulseCord in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export const isSafeMode = CommandLine.values["safe-mode"] === true;
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`PulseCord v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            PulseCord v${app.getVersion()}
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("PulseCord is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkCommandLineForQueryCommands() {
    ...
    const responseFile = join(tmpdir(), `pulsecord-query-${Date.now()}-${process.pid}.tmp`);
    ...
    console.error("PulseCord is not running. Query commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("PulseCord is already running. Quitting previous instance...");
            return;
        } else {
            console.log("PulseCord is already running. Quitting...");
            app.exit(0);
        }
    }
