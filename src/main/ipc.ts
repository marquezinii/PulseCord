...
handleSync(IpcEvents.DEPRECATED_GET_VENCORD_PRELOAD_SCRIPT_PATH, () =>
    isSafeMode ? join(STATIC_DIR, "empty.js") : join(VENCORD_DIR, "preload.js")
);
handleSync(IpcEvents.GET_VENCORD_PRELOAD_SCRIPT, () =>
    isSafeMode ? "" : readFileSync(join(VENCORD_DIR, "preload.js"), "utf-8")
);
handleSync(IpcEvents.GET_VENCORD_RENDERER_SCRIPT, () =>
    isSafeMode ? "" : readFileSync(join(VENCORD_DIR, "renderer.js"), "utf-8")
);

const VESKTOP_RENDERER_JS_PATH = join(__dirname, "renderer.js");
const VESKTOP_RENDERER_CSS_PATH = join(__dirname, "renderer.css");
handleSync(IpcEvents.GET_VESKTOP_RENDERER_SCRIPT, () =>
    isSafeMode ? "" : readFileSync(VESKTOP_RENDERER_JS_PATH, "utf-8")
);
handle(IpcEvents.GET_VESKTOP_RENDERER_CSS, () =>
    isSafeMode ? Promise.resolve("") : readFile(VESKTOP_RENDERER_CSS_PATH, "utf-8")
);
...
handleSync(IpcEvents.GET_GIT_HASH, () => PULSECORD_GIT_HASH);
