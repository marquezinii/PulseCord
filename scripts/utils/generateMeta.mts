...
const releases = await fetch("https://api.github.com/repos/marquezinii/PulseCord/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "app.pulsecord.desktop.metainfo.xml");
...
await fs.writeFile("./dist/app.pulsecord.desktop.metainfo.xml", output, "utf-8");
console.log("Updated meta information written to ./dist/app.pulsecord.desktop.metainfo.xml");
